...
const handleClick = () => {
  ...
  return (
    <article onClick={handleClick} className={`cards border-${poke?.types[0].type.name}`}>
        <header className= {`poke_header bg-${poke?.types[0].type.name}`}>
            <img className='poke_img' src={poke?.sprites.other['official-artwork'].front_default}
            alt="" />
        </header>
        <h2 className={`poke_name color-${poke?.types[0].type.name}`} >{poke?.name}</h2>
        <ul className='poke_type'>
            {
                poke?.types.map(type => (
                    <li className='poke_li' key={type.type.name}>{type.type.name}</li>
                ))
            }
        </ul>
